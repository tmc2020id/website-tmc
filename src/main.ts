...
// بيانات Odoo الوهمية (Mock Data)
const odooServices = [
  { 
    id: 101, 
    display_name: "تصميم الهوية البصرية (Branding)", 
    list_price: 500.00, 
    description_sale: "نبني لك هوية بصرية متكاملة تعكس رؤية شركتك وتجذب عملاءك.", 
    image_1920: "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==" // صورة تجريبية (بكسل أحمر)
  }, 
  { 
    id: 102, 
    display_name: "تطوير تطبيقات الويب (Web App)", 
    list_price: 1200.00, 
    description_sale: "تطبيقات ويب سريعة، آمنة، ومبنية بأحدث التقنيات.", 
    image_1920: "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==" // صورة تجريبية (بكسل أزرق)
  } 
];
...
odooServices.forEach((service, index) => {
  const material = createCardMaterial(service.image_1920);
  const mesh = new THREE.Mesh(cardGeometry, material);
  mesh.castShadow = true;
  mesh.receiveShadow = true;
  
  // توزيع البطاقات بشكل مبدئي
  const xPos = index === 0 ? -2 : 2;
  const yPos = 5 + index * 2;
  
  scene.add(mesh);

  const body = new CANNON.Body({
    mass: index === 0 ? 1 : 2, // البطاقة الثانية أثقل قليلاً
...
const openCardModal = (data: any) => {
  ...
  modalDesc.textContent = data.description_sale;
  modalPrice.textContent = `${data.list_price} ر.س`;
